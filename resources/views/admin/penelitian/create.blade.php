...
@section('title', 'Tambah Laporan Penelitian - LPPM')
...
<div class="mb-4">
    ...
    <h5 class="fw-bold mt-2">Tambah Laporan Penelitian Baru</h5>
</div>
...
<div class="glass-card">
    ...
    <form action="{{ route('admin.penelitian.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" id="title" class="form-control" placeholder="Analisis Pengaruh Good Corporate Governance Terhadap Nilai Perusahaan..." value="{{ old('title') }}" required>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <select name="scheme" id="scheme" class="form-select" required>
                    ...
                    <option value="Penelitian Dosen Pemula (PDP)" {{ old('scheme') === 'Penelitian Dosen Pemula (PDP)' ? 'selected' : '' }}>Penelitian Dosen Pemula (PDP)</option>
                    <option value="Penelitian Fundamental" {{ old('scheme') === 'Penelitian Fundamental' ? 'selected' : '' }}>Penelitian Fundamental</option>
                    <option value="Hibah Internal Kwik Kian Gie" {{ old('scheme') === 'Hibah Internal Kwik Kian Gie' ? 'selected' : '' }}>Hibah Internal Kwik Kian Gie</option>
                    <option value="Penelitian Terapan" {{ old('scheme') === 'Penelitian Terapan' ? 'selected' : '' }}>Penelitian Terapan</option>
                </select>
            </div>
        ...
        <div class="mb-3">
            ...
            <span class="text-white-50 d-block mt-1" style="font-size: 0.75rem;"><i class="fa-solid fa-circle-info me-1"></i> Berkas laporan lengkap hasil penelitian. Format harus PDF, maksimal 5 MB.</span>
        </div>

        <div class="mb-4">
            <label for="abstract" class="form-label">Abstrak Penelitian (Opsional)</label>
            <textarea name="abstract" id="abstract" class="form-control" rows="6" placeholder="Tuliskan abstrak atau ringkasan metodologi riset...">{{ old('abstract') }}</textarea>
        </div>

        <button type="submit" class="btn btn-primary px-4 py-2.5">Simpan Laporan Penelitian</button>
    </form>
</div>
@endsection
